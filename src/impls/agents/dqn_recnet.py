...
class RecThinkingCritic(nn.Module):
    d_model: int = 256
...
class GCRecDiscreteCritic(nn.Module):    
    d_model: int = 256
    action_dim: int = 6
    thinking_steps: int = 3
    ensemble: bool = True
...
class GCDQNRecAgent(flax.struct.PyTreeNode):
    rng: Any
    network: Any
    ...
    def create(
        ...
        critic_def = GCRecDiscreteCritic(
            d_model=config['recurrent_hidden_size'],
